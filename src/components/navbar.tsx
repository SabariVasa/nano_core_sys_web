...
//     { name: "Training & Skilling", href: "/training-details" },
...
export function Navbar() {
  ...
  return (
    <motion.header
      ...
      transition={{ duration: 0.5 }}
      className={cn(
        "sticky top-0 z-50 w-full transition-all duration-300 navbar-container",
        scrolled ? "shadow-md" : "",
      )}
    >
      <div className="container flex h-20 items-center justify-between">
      ...
      <AnimatePresence>
        {isMenuOpen && isMobile && (
          <motion.div
            ...
            exit={{ opacity: 0, height: 0 }}
            transition={{ duration: 0.3 }}
            className="fixed inset-0 top-20 z-40 overflow-hidden bg-white shadow-lg"
          >
            <div className="container py-6">
              <motion.div
                initial="hidden"
                ...
                {navItems.map((item) => (
                  ...
                  >
                    <Link
                      href={item.href}
                      className={cn(
                        "flex items-center py-3 text-lg font-semibold border-b border-gray-100",
